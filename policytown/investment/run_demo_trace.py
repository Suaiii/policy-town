...
def _role_of(prompt: str) -> str:
    ...
    m = re.search(r"(?:你叫|你是)(.+?)。", prompt)
    return ("企业代表 " + m.group(1).strip()) if m else "企业代表"


def _kind_of(prompt: str) -> str:
    for k in ("fiscal", "industry", "company_tech", "market_risk"):
        if '"kind": "%s"' % k in prompt:
            return k
    return "company_plan"
...
def _summary(out: dict) -> str:
    if not out:
        return "（失败→降级）"
    if "action" in out:
        return "→ %s（conf=%.2f，请求下轮 %s 点，竞争=%s）" % (
            out.get("action"), out.get("confidence", 0),
            out.get("capital_request_next_round", 0), out.get("competition_response", "-"))
    return "direction=%s score=%d conf=%.2f" % (
        out.get("direction"), out.get("score", 0), out.get("confidence", 0))
...
def main() -> None:
    ...
    for stage_id in ("S1", "S2", "S3", "S4"):
        ...
        for e in assessments_trace + plans_trace:
            e["role"] = _role_of(e["prompt"])
            e["kind"] = _kind_of(e["prompt"])
        rounds.append({
            "stage_id": stage_id,
    ...
    write_json("trace_llm.json", json.dumps(trace, ensure_ascii=False, indent=2))
    write_md(trace)
    print("产物：trace_llm.json（全量含 prompt）/ trace_llm.md（可读路径）")
...
def write_md(trace: dict) -> None:
    lines = [
        "# 推演对话路径（deepseek-v4-flash · seed 42）",
        "",
        "本局企业组合：%s" % ", ".join(PICK),
        "历史校准基线：方向 %.2f / 时序 %.2f / 机制 %.2f / 路径 %.2f" % (
            trace["historical_baseline"]["direction_score"],
    ...
    for r in trace["rounds"]:
        ...
        for e in r["assessments_trace"]:
            ...
            lines.append("- **%s** [%s %.1fs] %s" % (
                e["role"], src, e["t"], _summary(e["output"])))
            if e["output"] and e["output"].get("reasoning_summary"):
                lines.append("  - 理由：%s" % e["output"]["reasoning_summary"][:120])
        lines.append("")
        lines.append("### ③ 玩家决策")
        ...
        for e in r["plans_trace"]:
            ...
            o = e["output"]
            if o:
                lines.append("- **%s** [%s %.1fs] %s → %s，下轮请求 %s 点，资源分配=%s，"
                             "风险预案=%s，竞争响应=%s" % (
                                 e["role"], src, e["t"], o.get("company_id"), o.get("action"),
                                 o.get("capital_request_next_round"),
                                 json.dumps(o.get("resource_allocation", {}), ensure_ascii=False),
                                 o.get("risk_response"), o.get("competition_response")))
            else:
                lines.append("- **%s** [%s %.1fs] %s" % (e["role"], src, e["t"], _summary(None)))
        lines.append("")
        lines.append("### ⑤ 结算要点")
        ...
        for d in sorted(r["settlement"]["deltas"], key=lambda x: abs(x["delta"]),
                        reverse=True)[:6]:
            lines.append("  - Δ%s %+.1f（%s）" % (d["metric_id"], d["delta"], d["reason_code"]))
        for m in r["settlement"]["new_messages"]:
            lines.append("  - 📩 新入收件箱：%s（%s → 政府）" % (m["type"], m["from"]))
        lines.append("")
    lines.append("## 终局对照")
    lines.append("```json")
    lines.append(json.dumps(trace["final"]["historical_replay"], ensure_ascii=False, indent=2))
    lines.append("```")
